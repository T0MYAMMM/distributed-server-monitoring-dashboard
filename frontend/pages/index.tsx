...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Real-time: subscribe to the backend WebSocket so the dashboard updates
    // the instant an agent reports. Reconnects automatically on drop.
    const connect = () => {
      if (closed) return
      try {
    ...
    // Initial load + slow polling as a fallback when the socket is unavailable.
    fetchServers()
    connect()
