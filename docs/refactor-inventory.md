...
| 3 | Frontend foundation (shell, tokens, client, WS, Query) | IN PROGRESS (branch `phase-3-frontend-foundation`). DONE: dependency overhaul (Chakra/emotion/framer/dnd/jsonwebtoken removed; TanStack Query+Table, Recharts, lucide, Radix, cmdk, shadcn utils added); CSS-variable design tokens + Tailwind theme (dark-first, orange accent); typed v1 API client; auth token module; WS manager (backoff reconnect); TanStack Query provider; hooks (useServers/useFleetSummary/useServerMetrics/useAlerts). Existing Pages Router app still builds (tsc + next build green). REMAINING: shadcn UI primitives + shared components (StatCard/StatusBadge/TrendBadge/ResourceBar/ChartPanel/DataTable), config/nav.ts, AppShell (Sidebar/Topbar/PageHeader/CommandPalette), App Router migration (app/ with (dashboard) layout + providers), and porting login/dashboard/admin into the shell (then delete pages/). |
